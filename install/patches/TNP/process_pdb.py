...
def fix_pdb_for_dssp(pdb_file):
    """
    Fix PDB file format for DSSP compatibility.
    
    NanoBodyBuilder2 and some other tools output PDB files with 79-character 
    lines instead of the standard 80 characters. mkdssp 3.x is strict about
    this and will fail to parse such files.
    
    This function creates a temporary fixed PDB file with proper formatting.
    ...
    -------
    str
        Path to the fixed temporary PDB file (caller should delete after use)
    """
    ...
    for line in lines:
        ...
        # For ATOM/HETATM records, ensure 80 character width
...
def run_dssp(structurefile, type=''):
    ...
    """
    # Fix PDB format for DSSP compatibility (NanoBodyBuilder2 outputs 79-char lines)
    fixed_pdb = fix_pdb_for_dssp(structurefile)
    ...
    try:
        if type == 'tcr':
            ...
            try:
                _ = DSSP(s[0], fixed_pdb) 
            except OSError: 
                # dssp is not installed so cannot be run.
                return {"dssp_properties": {"B":[], "A":[]}, "annotation_file": None, "dssp_message": "DSSP is not in the path"}
            ...
            try:
                ...
                # dssp is not installed so cannot be run.
                return {"dssp_properties": {"H":[], "L":[]}, "annotation_file": None, "dssp_message": "DSSP is not in the path"}
    finally:
        ...
        for chain in s[0]:
            for r in chain:
                if r.id[0] !="W":
                    ...
                    try:
                        print(",".join( [ chain.id, str(r.id[1]), r.id[2], protein_letters_3to1.get(r.get_resname(), 'X'), "%.2f"%r.xtra['EXP_DSSP_ASA'], "%.2f"%r.xtra['EXP_DSSP_RASA'],
